...
def check_nan_losses(loss):
    ...
    if math.isnan(loss):
        ...
        print("ERROR: Got NaN losses {}".format(datetime.now()))
        import sys
        sys.stdout.flush()
...
def get_model_stats(model, cfg, mode, use_train_input, model_inputs=None):
    """
...
def log_agent_model_info(agent, model, cfg, use_train_input=True):
    """
    Log info, includes number of parameters, gpu usage, gflops and activation count.
        The model info is computed when the model is in validation mode.
    Args:
    ...
    if get_num_gpus(cfg) <= 1:
        agent_output = agent(model_inputs)
        logger.info(
            "Agent Flops: {:,} G".format(
                get_model_stats(agent, cfg, "flop", use_train_input, model_inputs)
            )
        )
        logger.info(
            "Agent Activations: {:,} M".format(
                get_model_stats(agent, cfg, "activation", use_train_input, model_inputs)
            )
        )
        model_inputs['mask'] = agent_output['mask']
        logger.info(
            "Model Flops: {:,} G".format(
                get_model_stats(model, cfg, "flop", use_train_input, model_inputs)
            )
        )
        logger.info(
            "Model Activations: {:,} M".format(
                get_model_stats(model, cfg, "activation", use_train_input, model_inputs)
            )
        )
    logger.info("nvidia-smi")
    os.system("nvidia-smi")
...
def is_eval_epoch(cfg, cur_epoch, multigrid_schedule=None):
    ...
    if cfg.TRAIN.EVAL_PERIOD == 0:
        return False
    if cur_epoch + 1 >= cfg.OPTIMIZER.MAX_EPOCH - 10 and (not cfg.PRETRAIN.ENABLE) and (not cfg.MM_RETRIEVAL.ENABLE):
        return True
