...
class TrainMeter(object):
    ...
    def __init__(self, epoch_iters, cfg, max_epoch_override=None):
        """
        Args:
            epoch_iters (int): the overall number of iterations of one epoch.
            cfg (Config): the global config object.
        """
        self._cfg = cfg.deep_copy()
        self.epoch_iters = epoch_iters
        if max_epoch_override:
            self._cfg.OPTIMIZER.MAX_EPOCH = max_epoch_override
        self.MAX_EPOCH = self._cfg.OPTIMIZER.MAX_EPOCH * epoch_iters / self._cfg.TRAIN.NUM_FOLDS
        self.iter_timer = Timer()
    ...
    def update_stats(self, top1_err, top5_err, loss, lr, mb_size, **kwargs):
        ...
        if not self._cfg.PRETRAIN.ENABLE and not self._cfg.MM_RETRIEVAL.ENABLE:
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        eta_sec = self.iter_timer.seconds() * (
            self.MAX_EPOCH - (cur_epoch * self.epoch_iters / self._cfg.TRAIN.NUM_FOLDS + cur_iter + 1)
        )
        eta = str(datetime.timedelta(seconds=int(eta_sec)))
        stats = {
            "_type": "train_iter",
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        """
        eta_sec = self.iter_timer.seconds() * (
            self.MAX_EPOCH - (cur_epoch + 1) * self.epoch_iters / self._cfg.TRAIN.NUM_FOLDS
        )
        eta = str(datetime.timedelta(seconds=int(eta_sec)))
        stats = {
            "_type": "train_epoch",
