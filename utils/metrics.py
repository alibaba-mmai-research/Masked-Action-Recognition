...
def topk_retrieved_correct_video_level_loose(retrieved_labels, labels, ks):
    correct_matrix = labels.view(-1, 1, 1).expand_as(retrieved_labels) == retrieved_labels
    topks_correct = [
        correct_matrix[:, :, :k].sum(-1).sum(-1) > 0 for k in ks
    ]
    return topks_correct

def topk_retrieved_correct_clip_level_tight(retrieved_labels, labels, ks):
    correct_matrix = labels.view(-1, 1, 1).expand_as(retrieved_labels) == retrieved_labels
    topks_correct = [
        correct_matrix[:, :, :k].sum(-1) > 0 for k in ks
    ]
    return topks_correct