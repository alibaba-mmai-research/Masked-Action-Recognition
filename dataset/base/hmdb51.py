...
class Hmdb51(BaseVideoDataset):
    ...
    def _get_dataset_list_name(self):
        ...
        if hasattr(self.cfg.TRAIN, "FEW_SHOT") and self.cfg.TRAIN.FEW_SHOT and self.split == "train":
            name = "hmdb51_train_list_fewshot_{}.txt".format(
                3 if not hasattr(self.cfg.TRAIN, "FEW_SHOT_NUM") else self.cfg.TRAIN.FEW_SHOT_NUM
            )
            print("FEW SHOT!!")
        else:
    ...
    def _config_transform(self):
        ...
        elif self.split == 'linear_train_cls':
            if self.cfg.DATA.TRAIN_JITTER_SCALES[0] <= 1:
                scale = self.cfg.DATA.TRAIN_JITTER_SCALES
            else:
                scale = [
